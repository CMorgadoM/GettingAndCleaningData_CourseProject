...
y_train <- y_train %>% merge(act_lab) %>% select(V2) %>% rename(activity_name = V2)
...
# a) replace the activity numbers with their correspondent names
y_test <- y_test %>% merge(act_lab) %>% select(V2) %>% rename(activity_name = V2)
...
# c) Erase used objects
rm(sub_test, x_test, y_test,dir_subject_test, dir_X_test, dir_Y_test, act_lab)
...
write_csv(codebook, file = "C:/Users/juice/final_dataset.csv")